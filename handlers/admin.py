...
async def manage_admin(message: types.Message, state: FSMContext):
...
async def get_data_admin(message: types.Message):
    await message.answer('Пока эта функция не реализована')
...
def register_handlers_admin(dp: Dispatcher):
    ...
    dp.register_message_handler(get_data_admin, lambda message: message.text.startswith('Выгрузить'), state=FSM_client.admin)
